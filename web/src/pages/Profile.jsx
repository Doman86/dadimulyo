          ...
          <div className="card-lux p-6 !rounded-2xl">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">
                  {user.role?.name ? user.role.name.charAt(0).toUpperCase() + user.role.name.slice(1).replace('_', ' ') : 'Customer'}
                </p>
              </div>
            </div>
            ...
            <dl className="mt-6 space-y-4 text-sm">
              <div className="flex flex-col gap-1 border-b border-gray-100 pb-4">
                <dt className="font-medium text-gray-400 text-xs uppercase tracking-wide">Email</dt>
                <dd className="text-charcoal font-medium">{user.email}</dd>
              </div>
              {user.phone && (
                <div className="flex flex-col gap-1 border-b border-gray-100 pb-4">
                  <dt className="font-medium text-gray-400 text-xs uppercase tracking-wide">Telepon</dt>
                  <dd className="text-charcoal font-medium">{user.phone}</dd>
                </div>
              )}
              <div className="flex flex-col gap-1 border-b border-gray-100 pb-4">
                <dt className="font-medium text-gray-400 text-xs uppercase tracking-wide">Status</dt>
                <dd><span className="badge-green">{user.status || 'active'}</span></dd>
              </div>
              <div className="flex flex-col gap-1">
                <dt className="font-medium text-gray-400 text-xs uppercase tracking-wide">Terdaftar sejak</dt>
