...
export default function Wishlist() {
  ...
  useEffect(() => {
    fetchWishlists()
      ...
      .catch(() => setError(t('wishlist.load_failed')))
      .finally(() => setLoading(false));
  }, []);

  async function handleRemove(truckId) {
    try { await toggleWishlist(truckId); setTrucks((prev) => prev.filter((item) => item.id !== truckId)); } catch { setError(t('wishlist.remove_failed')); }
  }
    ...
    <div>
      <section className="page-hero !py-12">
        <div className="relative z-10">
          <Reveal><h1 className="font-display text-3xl font-extrabold text-white">{t('wishlist.title')}</h1></Reveal>
          <Reveal variant="up" delay={100}><p className="mt-2 text-white/50 text-sm">{t('wishlist.desc')}</p></Reveal>
        </div>
      </section>
        ...
        {loading ? (
          ...
          <Reveal variant="zoom">
            <div className="py-16 text-center">
              ...
              <p className="mt-4 text-gray-500 text-lg">{t('wishlist.empty')}</p>
              <Link to="/trucks" className="mt-4 inline-flex btn-outline-lux rounded-full px-6 py-2.5 text-sm font-bold">{t('wishlist.browse')}</Link>
            </div>
          </Reveal>
        ) : (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div className="card-lux card-img-zoom overflow-hidden">
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <div className="text-xs text-gray-400">{truck.year} · {truck.category?.name || t('nav.trucks')}</div>
                      ...
                      <button onClick={() => handleRemove(truck.id)} className="shrink-0 rounded-lg px-3 py-1.5 text-xs font-bold text-red-500 hover:bg-red-50 transition-colors" title={t('wishlist.remove_title')}>
                        {t('wishlist.remove')}
                      </button>
                    </div>
                  </div>
