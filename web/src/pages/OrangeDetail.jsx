...
export default function OrangeDetail() {
  ...
  if (notFound || !product) return (
    <div className="py-20 text-center">
      ...
      <Link to="/oranges" className="mt-4 inline-flex btn-outline-lux rounded-full px-6 py-2.5 text-sm font-bold">
        Kembali ke katalog
      </Link>
    </div>
  );
      ...
      <div className="mx-auto max-w-7xl px-4 py-10">
          ...
          <Reveal variant="right" className="lg:col-span-2">
            <div className="sticky top-24">
              <div className="flex items-center gap-2 text-sm text-gray-400">
                ...
                {outOfStock ? (
                  <span className="badge-orange">Stok Habis</span>
                ) : (
                  <span className="badge-green">Stok Tersedia</span>
                )}
              </div>
              ...
              <div className="mt-5 rounded-2xl bg-gradient-to-br from-sand to-cream p-5 border border-gold/10">
                <div className="flex items-baseline justify-between">
                  <span className="text-sm text-gray-500">Harga eceran</span>
                ...
                {product.wholesale_price && (
                  <div className="mt-2 flex items-baseline justify-between border-t border-gold/10 pt-2">
                    <span className="text-sm text-gray-500">Harga grosir</span>
              ...
              <dl className="mt-5 grid grid-cols-2 gap-3">
                {[
                  { label: 'Stok', value: `${formatNumber(product.stock_kg)} kg` },
                  { label: 'Min. Order', value: `${formatNumber(product.minimum_order_kg)} kg` },
                  {
                    label: 'Jadwal Panen',
                    value: product.harvest_date
                      ? new Date(product.harvest_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })
                      : '-'
                  },
                  { label: 'Lokasi Kebun', value: product.farm_location || '-' },
                ].map((item) => (
              ...
              <div className="mt-5 rounded-2xl bg-gradient-to-br from-primary-dark to-forest p-5 text-white">
                <h3 className="font-semibold">Butuh pesanan dalam jumlah besar?</h3>
        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-2">
            ...
            <section className="card-lux p-6 !rounded-2xl">
              ...
              {product.seller && (
                <p className="mt-4 text-sm text-gray-400">
                  Dijual oleh: <span className="font-semibold text-charcoal">{product.seller.name}</span>
                </p>
              )}
            </section>
          </Reveal>
          <Reveal variant="right">
            <section className="card-lux p-6 !rounded-2xl">
              <h2 className="font-display text-xl font-bold text-primary">Ulasan</h2>
              {product.reviews?.length ? (
                ...
                <p className="mt-3 text-gray-400">Belum ada ulasan.</p>
              )}
            </section>
          </Reveal>
        </div>
